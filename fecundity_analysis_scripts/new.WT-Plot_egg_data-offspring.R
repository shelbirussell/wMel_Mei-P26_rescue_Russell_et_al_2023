...
all_F_offspring <- rbind(WT_OreR_wMel_offspring, WT_OreR_uninf_offspring, WT_F10_OreR_uninf_offspring, WT_Gal4CyO_wMel_offspring, WT_Gal4CyO_uninf_offspring, WT_Gal4Sb_wMel_offspring, WT_Gal4Sb_uninf_offspring, RFP_wMel_offspring, RFP_uninf_offspring)
## this produces a data frame with samples in the intended order
all_F_offspring_levels <- c(paste("WT_OreR_wMel"), paste("WT_OreR_uninf"), paste("WT_F10_OreR_uninf"), paste("WT_Gal4CyO_wMel"), paste("WT_Gal4CyO_uninf"), paste("WT_Gal4Sb_wMel"), paste("WT_Gal4Sb_uninf"), paste("RFP_wMel"), paste("RFP_uninf"))
...
par(mar=c(9,12,4,5))
boxplot(all_F_offspring$value ~ all_F_offspring$group, outline=FALSE, col="white", ylim=c(0,100), xlab="", ylab="# offspring / female / day", names=FALSE)
...
text(x=1:length(all_F_offspring_levels), y=par("usr")[3]-25, labels=all_F_offspring_levels, xpd=NA, srt=45, cex=1)
dev.off()


########################################
#######################################
recip_all_F_offspring <- rbind(WT_OreR_wMel_offspring, WT_OreR_wMel_recip_offspring, WT_OreR_uninf_offspring, WT_F10_OreR_uninf_offspring, WT_Gal4CyO_wMel_offspring, WT_Gal4CyO_uninf_offspring, WT_Gal4Sb_wMel_offspring, WT_Gal4Sb_uninf_offspring, RFP_wMel_offspring, RFP_uninf_offspring)
## this produces a data frame with samples in the intended order
recip_all_F_offspring_levels <- c(paste("WT_OreR_wMel"), paste("WT_OreR_wMel_recip"), paste("WT_OreR_uninf"), paste("WT_F10_OreR_uninf"), paste("WT_Gal4CyO_wMel"), paste("WT_Gal4CyO_uninf"), paste("WT_Gal4Sb_wMel"), paste("WT_Gal4Sb_uninf"), paste("RFP_wMel"), paste("RFP_uninf"))
## reorder by group for beeswarm
recip_all_F_offspring$group <- factor(recip_all_F_offspring$group, levels=recip_all_F_offspring_levels)

pdf("recip.all-WT_offspring_3-7d_beeswarm_boxplot.pdf")
par(mar=c(9,12,4,5))
boxplot(recip_all_F_offspring$value ~ recip_all_F_offspring$group, outline=FALSE, col="white", ylim=c(0,100), xlab="", ylab="# offspring / female / day", names=FALSE)
beeswarm(recip_all_F_offspring$value ~ recip_all_F_offspring$group, pwcol=recip_all_F_offspring$color, pch=19, method="swarm", corral="wrap", priority="density", labels=FALSE, add=TRUE, cex=1)
text(x=1:length(recip_all_F_offspring_levels), y=par("usr")[3]-25, labels=recip_all_F_offspring_levels, xpd=NA, srt=45, cex=1)
dev.off()
